  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/6 w-72 h-72 rounded-full pointer-events-none"
        style={{ background: 'rgba(79,142,247,0.06)', filter: 'blur(100px)' }} />
      <div className="absolute bottom-1/3 right-1/4 w-64 h-64 rounded-full pointer-events-none"
        style={{ background: 'rgba(139,92,246,0.06)', filter: 'blur(90px)' }} />

      {/* 3D Scene — right side, pointer-events none */}
        ...
        >
          ...
          <motion.div variants={item} className="flex flex-wrap gap-7">
            {[
              ...
              { value: '320+', label: 'LeetCode Solved' },
              { value: '100', label: 'Day Streak 🔥' },
            ].map(stat => (
              <div key={stat.label}>
                <div className="font-display font-bold text-2xl gradient-text">{stat.value}</div>
                <div className="text-xs mt-0.5 font-mono" style={{ color: 'var(--text-muted)' }}>{stat.label}</div>
              </div>
            ))}
          </motion.div>
        </motion.div>
